...
func colToFieldIdxChain(m *DbMap, t reflect.Type, i interface{}, cols []string) (
	...
	colsToFields := make(map[string][]reflect.StructField)
	colIndexChain := make([][][]int, len(cols))
	...
	if len(missingColNames) > 0 {
		return colsToFields, colIndexChain, &NoFieldInTypeError{
			TypeName:        t.Name(),
			MissingColNames: missingColNames,
		}
	}
	return colsToFields, colIndexChain, nil
}
